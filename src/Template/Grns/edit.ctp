					...
					<div class="col-md-8">
						<table id="main_table" class="table table-condensed table-bordered">
							...
							foreach($grn_details as $grn_detail){
							 ...
							 $item_name=$grn_detail->item->name;
							 $minimum_quantity_factor=$grn_detail->item->minimum_quantity_factor;
							 $unit_name=$grn_detail->item->unit->unit_name;
							 $msg_show=$minimum_quantity_factor*$quantity;
							?>
					<tr class="main_tr" class="tab">
					<td align="center" width="1px"></td>
				    <td>
						<?php echo $this->Form->input('item_id', ['empty'=>'--Select-','options'=>$items,'label' => false,'class' => 'form-control input-sm attribute', 'value'=>$item_id]); ?>
						<span class="msg_shw" style="color:blue;font-size:12px;">Selling Factor in : <?php echo $unit_name; ?></span>
					</td>
					<td>
						<?php echo $this->Form->input('quantity', ['label' => false,'class' => 'form-control input-sm number quant','placeholder'=>'Quantity', 'value'=>$quantity, 'unit_name'=>$unit_name]); ?>
						<span class="msg_shw2" style="color:blue;font-size:12px;"><?php echo $msg_show.' '.$unit_name; ?></span>	
					</td>
                    <td>
