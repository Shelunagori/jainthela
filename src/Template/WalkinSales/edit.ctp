					...
					<!-- BEGIN FORM-->
						...
						<table id="main_table" class="table table-condensed table-bordered">
							...
							<tbody id='main_tbody' class="tab">
								<?php 
								$k=0;
 								foreach($WalkinSaleDetails as $WalkinSaleDetail){ 
								$fetch_item_id=$WalkinSaleDetail->item_id;
								$fetch_unit_name=$WalkinSaleDetail->item->unit->unit_name;
								$fetch_quantity=$WalkinSaleDetail->quantity;
								$fetch_rate=$WalkinSaleDetail->rate;
								$fetch_amount=$WalkinSaleDetail->amount;
								?>
				<tr class="main_tr" class="tab">
					<td align="center" width="1px"></td>
				    <td>
						<?= $this->Form->input('item_id',array('options' => $items,'class'=>'form-control input-sm attribute','empty' => 'Select','label'=>false, 'value'=>$fetch_item_id)) ?>
						<span class="msg_shw" style="color:blue;font-size:12px;">selling factor in :<?php echo $fetch_unit_name; ?></span>
						
						<?php echo $this->Form->input('walkin_sale_details.'.$k.'.id', ['value' => $fetch_item_id]); ?>
					</td>
					<td>
						<?php echo $this->Form->input('quantity', ['label' => false,'class' => 'form-control input-sm number calculation_amount quant','placeholder'=>'Quantity', 'value'=>$fetch_quantity]); ?>
						<span class="msg_shw2" style="color:blue;font-size:12px;"><?php echo $fetch_quantity.' '.$fetch_unit_name; ?></span>
					</td>
					<td>
						<?php echo $this->Form->input('rate', ['label' => false,'class' => 'rat_value form-control input-sm calculation_amount number','placeholder'=>'Price', 'value'=>$fetch_rate]); ?>
					</td>
                    <td>
						<?php echo $this->Form->input('amount', ['label' => false,'class' => 'form-control input-sm number','placeholder'=>'Total Amount', 'value'=>$fetch_amount]); ?>
					</td>
                    <td>
